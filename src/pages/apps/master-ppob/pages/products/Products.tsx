...
const Products = () => {
    ...
    const onSubmit = handleSubmit(async (data) => {
        ...
        if (editForm) {
            response = await dispatch(editProductsData(data?.id, payload));
            if (response) {
                handleClose();
    ...
    const handleGetProductsByGroupData = async (event: SelectChangeEvent) => {
        const value = event.target.value as string
        setGroupId(value);
                ...
                <div className="flex flex-col lg:flex-row lg:justify-between gap-4 lg:items-center">
                    ...
                    <div className='flex items-center gap-3 justify-between'>
                        ...
                        <div className='flex items-center gap-3'>
                            ...
                            {productGroupList && productGroupList?.length > 0 && <FormControl sx={{ minWidth: 220 }} size="small">
                                        ...
                                        {rows?.map((row, index) => {
                                            ...
                                            return (
                                                <TableRow hover tabIndex={-1} key={row.id}>
                                                    ...
                                                    <TableCell align="left">
                                                        {currencyFormat(row.harga_beli ?? 0)}
                                                    </TableCell>
                                                    <TableCell align="left">
                                                        {currencyFormat(row.harga_jual ?? 0)}
                                                    </TableCell>
                                        ...
                                        <div className="flex flex-col gap-3 w-full">
                                            ...
                                            <Autocomplete
                                                ...
                                                isOptionEqualToValue={(option, val) => {
                                                    return (
                                                        option?.id === getProductsListItem(val)?.id
                                                    );
                                                }}
                                                fullWidth
