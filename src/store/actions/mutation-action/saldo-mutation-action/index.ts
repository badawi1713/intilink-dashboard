    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        ...
        if (axiosError.response) {
          ...
          dispatch(setSaldoMutationError(error.message || 'Maaf, sedang terjadi kesalahan'));
        }
      } else {
        dispatch(
          showMessage({
...
const handleGetSaldoMutationData = (): AppThunk => async (dispatch, getState: () => RootState) => {
  ...
  try {
    ...
    if (axios.isAxiosError(error)) {
      ...
      if (axiosError.response) {
        const { message, code } = axiosError.response.data;
        dispatch(setSaldoMutationError(`${code}: ${message}` || 'Maaf, sedang terjadi kesalahan'));
      } else {
        dispatch(setSaldoMutationError(error.message || 'Maaf, sedang terjadi kesalahan'));
      }
    } else {
